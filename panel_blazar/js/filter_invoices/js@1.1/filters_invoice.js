...
function download_invoices_filter() {
    ...
    const filter_array=generate_filter_invoices(filter_download,json_invoices);
    const result_data=[];
   for(var y=0; y<filter_array.length; y++){
       result_data.push({"Referencia":filter_array[y].invoicenum,"Fecha de referencia":filter_array[y].normalisedDateCreated,"Fecha de vencimiento":filter_array[y].normalisedDateDue,"Estatus":filter_array[y].statustext,"Total":filter_array[y].totalnum.toFixed(2)})
   }
   const data_filter=JSON.stringify(result_data);
